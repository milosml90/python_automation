...
async def pingTest(target, file):
    ...
    if sys.platform == 'win32':
        ...
        input_with_timeout("Input the name of the operator, e.g. vipmobile: ", 10)
...
while (True):
    if (isPrinted):
        ...
        if (i == 1):
            ...
            # pingTest("8.8.8.8" + " ".join(repeatCommand(command, 100)), file[6])
            # pingTest("www.google.com" + " ".join(repeatCommand(command, 50)), file[7])
            ...
            pingTest("8.8.8.8", file[6])
            pingTest("www.google.com", file[7])
